...
def build_relationship_dossiers(state: MemoryState, llm_processor: Any) -> List[RelationshipDossier]:
    ...
    for index, person_id in enumerate(state.face_db.keys()):
        if person_id == primary_person_id:
            continue
        screening = (state.screening or {}).get(person_id)
        if screening and screening.memory_value == "block":
            continue
        candidates.append((index, person_id, screening))

    if not candidates:
        return []

    ordered_dossiers: List[RelationshipDossier | None] = [None] * len(candidates)
    max_workers = min(8, max(1, cpu_count() or 1), len(candidates))
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_map = {
            executor.submit(_build_single_relationship_dossier, person_id, screening, state, llm_processor): slot_index
            for slot_index, (_, person_id, screening) in enumerate(candidates)
        }
        for future in as_completed(future_map):
            slot_index = future_map[future]
            ordered_dossiers[slot_index] = future.result()
    return [dossier for dossier in ordered_dossiers if dossier is not None]
...
def infer_relationships_from_dossiers(
    ...
    *,
    ...
    ordered_relationships: List[RelationshipRecord | None] = [None] * len(dossiers)
    ordered_dossiers: List[RelationshipDossier | None] = [None] * len(dossiers)
    completed = 0
    kept_count = 0
    with ThreadPoolExecutor(max_workers=min(4, len(dossiers))) as executor:
        future_map = {
            executor.submit(_infer_relationship_unit, dossier, llm_processor): index
            for index, dossier in enumerate(dossiers)
        }
        for future in as_completed(future_map):
            index = future_map[future]
            relationship, updated_dossier = future.result()
            ordered_relationships[index] = relationship
            ordered_dossiers[index] = updated_dossier
            completed += 1
                ...
                progress_callback(
                    {
                        ...
                        "current_candidate_index": completed,
                        "last_completed_person_id": updated_dossier.person_id,
                        "relationship_count": kept_count,
                    }
                )
    updated_dossiers = [dossier for dossier in ordered_dossiers if dossier is not None]
    relationships = [
        relationship
        for relationship, dossier in zip(ordered_relationships, updated_dossiers)
        if relationship is not None and dossier.retention_decision == "keep"
    ]
...
def _build_single_relationship_dossier(
    person_id: str,
    screening: Any,
    state: MemoryState,
    llm_processor: Any,
...
def _infer_relationship_unit(
    dossier: RelationshipDossier,
    llm_processor: Any,
) -> Tuple[RelationshipRecord, RelationshipDossier]:
    ...
    return relationship, updated_dossier
