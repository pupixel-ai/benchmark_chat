...
class ProfileAgent:
    ...
    def _run_batch(
        ...
        if llm_processor is None or not hasattr(llm_processor, "_call_llm_via_official_api"):
            return self._run_batch_offline(batch)
...
6. 证据引用只需要可追溯锚点（event_id/photo_id/person_id/feature_name），不要重复展开整包原始证据。"""
    ...
    def _build_draft(self, field_key: str, field_output: Dict[str, Any], tool_trace: Dict[str, Any]) -> Dict[str, Any]:
        ...
        draft_value = field_output.get("draft_value", field_output.get("value"))
        draft_reasoning = _build_reasoning_from_ids(
            field_key,
            draft_value,
            evidence,
            explicit_reason=field_output.get("draft_reasoning") or field_output.get("reasoning"),
        )
        return {
            "value": draft_value,
