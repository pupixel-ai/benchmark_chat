...
class V0325PipelineTests(unittest.TestCase):
    ...
    def test_v0325_pipeline_can_resume_from_precomputed_vp1_and_lp1(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            profile_fact_decisions = json.loads((task_dir / "v0325" / "profile_fact_decisions.json").read_text(encoding="utf-8"))
            self.assertTrue(profile_fact_decisions)
            self.assertIn("supporting_ids", profile_fact_decisions[0])
            self.assertNotIn("tool_trace", profile_fact_decisions[0])
            self.assertFalse((task_dir / "v0325" / "profile_fact_decisions.full.json.gz").exists())
